...
def main(_):
    ...
    # our own split.
    xml_files = [f for f in listdir(annotations_dir) if isfile(join(annotations_dir, f))]
    random.seed(42)
    random.shuffle(xml_files)
    num_examples = len(xml_files)
    num_train = int(0.8 * num_examples)
    train_examples = xml_files[:num_train]
    val_examples = xml_files[num_train:]
    logging.info('%d training and %d validation examples.',
                 len(train_examples), len(val_examples))

    train_output_path = os.path.join(FLAGS.output_dir, 'train.record')
    val_output_path = os.path.join(FLAGS.output_dir, 'test.record')
